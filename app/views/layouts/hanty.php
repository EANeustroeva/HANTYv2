...
<head>
    ...
    <link href="/style.css?r=1" rel="stylesheet">
<style>
    #script-warning {
        display: none;
        background: #eee;
...
<body>
    <nav class="navbar navbar-light bg-white p-0" style="z-index: 100;">
        ...
        <div class="ml-auto">
            ...
            <img class="ml-3" src="/img/nav_top.svg" alt="">
        </div>
    </nav>
        ...
        <div class="sidebar">
            <ul>
                <?php foreach ($menu as $url => $item) : ?>
                    <li style="line-height: 2;" class="mb-4 pr-5">
                        <a class="d-flex justify-content-between w-100 <?= $url == $route['action'] ? 'active' : '' ?>" href="<?= $url == 'index' ? '/' : $url ?>">
                            <i class="<?= $item ?>"></i>
                ...
                <li><a href="#" data-toggle="modal" data-target="#config"><img src="/img/config.png" alt="" style="width:1.2em"></a></li>
    ...
    <div class="modal fade" id="config" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content" style="display: block;text-align: center;padding: 1em;">
                <img src="/img/edit.svg" class="img-fluid" alt="">
            </div>
        </div>
    </div>
...
<script>
    ...
    $('tbody > tr:not(.sel)').click(function() {
        $('tbody > tr.sel').removeClass('sel')
        $(this).addClass('sel')
    })
</script>
            ...
            JSON.parse(res).forEach(function(val, i, arr) {
                if ($('#nv').val().indexOf(val) !== -1) {
                    ...
                    if ($('#nv').find("option[value='" + data.id + "']").length) {
                        ...
                        $('#nv').append(newOption).trigger('change');
                    } 
                }
